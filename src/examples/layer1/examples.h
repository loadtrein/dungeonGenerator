...
#include "labyrithn/labyrithn_app.h"
#include "lsystem/lsystem.h"
#include "landscape/landscape.h"
#include "fluids/fluids_app.h"
#include "dungeon_generator/dungeonModel.h"
#include "dungeon_generator/dungeon_generator.h"
...
namespace octet {
  static app *app_factory(const char *name, int argc, char **argv) {
    if (!strcmp(name, "triangle")) return new triangle_app(argc, argv);    //  triangle sample: start here to learn how to draw a triangle
	  ...
	  else if (!strcmp(name, "labyrithn")) return new labyrithn_app(argc, argv); //  labyrithn
	  else if (!strcmp(name, "lsystem")) return new LSystemHandler(argc, argv); //  lsystem
    else if (!strcmp(name, "landscape")) return new Landscape(argc, argv); //  landscape
    else if (!strcmp(name, "fluids")) return new fluids_app(argc, argv); //  fluids
    else if (!strcmp(name, "dungeons")) return new dungeon_generator(argc, argv); //  dungeon_generator
    else return 0;
  }
  ...
  inline void run_examples(int argc, char **argv) {
    ...
    if (argc == 1) {
      ...
      app *myapp = app_factory("dungeons", argc, argv);
